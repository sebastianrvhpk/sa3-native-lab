...
## Tuning, Pitch, And Relational MIR

### pYIN

- Source: [Mauch and Dixon, pYIN, ICASSP 2014](https://webspace.eecs.qmul.ac.uk/s.e.dixon/pub/2014/MauchDixon-PYIN-ICASSP2014.pdf)
- Relevant idea: f0 extraction should keep uncertainty and candidate structure before smoothing; a single framewise pitch can discard useful alternate interpretations.
- Notebook impact: `tuning_maps.py` starts dependency-light but keeps confidence, event segmentation, and uncertainty rows rather than pretending f0 is ground truth.
- Status: source context; local pitch tracker is a lightweight microscope, not a pYIN replacement.

### CREPE

- Source: [CREPE](https://arxiv.org/abs/1802.06182)
- Relevant idea: learned waveform pitch trackers can outperform DSP heuristics on monophonic pitch estimation and noise robustness.
- Notebook impact: CREPE-style or similar learned f0 extraction is a future optional validation path, but local implementation starts with deterministic rows so SA3/SAME claims remain auditable.
- Status: source context; optional future adapter/procedure only.

### Basic Pitch

- Source: [Basic Pitch paper](https://arxiv.org/abs/2203.09893) and [official project](https://basicpitch.spotify.com/about)
- Relevant idea: modern lightweight MIR can produce polyphonic note and pitch-bend evidence across diverse instruments.
- Notebook impact: polyphonic and pitch-bend extraction should eventually validate real-world tuning maps, but it should enter as evidence rows rather than an external semantic judge.
- Status: source context; optional future adapter/procedure only.

### Wilson Combination Product Sets

- Source: [Wilson Archives](https://www.anaphoria.com/wilson.html) and [Combination Product Sets](https://anaphoria.com/cps.pdf)
- Relevant idea: CPS structures such as the hexany, dekany, and eikosany are relational graphs generated by products of factor subsets, not tonic-centered scale labels.
- Notebook impact: `tuning_maps.py` includes CPS fit rows so audio pitch centers can be compared against Wilson-style vertex sets without claiming the piece "is" a CPS.
- Status: source context; local CPS rows are microscope evidence.

### Erlich Harmonic Entropy

- Source: [Harmonic Entropy appendix](https://sethares.engr.wisc.edu/paperspdf/HarmonicEntropy.pdf) and [Paul Erlich papers page](https://sethares.engr.wisc.edu/papers/erlich.html)
- Relevant idea: consonance/tonalness can be framed as uncertainty over nearby simple integer ratios.
- Notebook impact: `tuning_maps.py` includes an Erlich-inspired entropy proxy over low-integer ratio candidates; this is a practical row, not a full psychoacoustic model.
- Status: source context; local implementation is an approximation for evidence packets.

### Regular Temperament Theory

- Source: [Regular temperament overview](https://en.xen.wiki/w/Regular_temperament), with local interpretation informed by the Keenan/Smith regular mapping tradition.
- Relevant idea: pitch systems can be represented by periods, generators, mappings, monzos, vals, and tempered commas rather than only by named scales.
- Notebook impact: `tuning_maps.py` returns period-plus-generator candidates and monzo coordinates for ratio edges. These become scalar/vector targets for later SAME/SA3 probes.
- Status: source context; local generator rows are compact-coordinate hypotheses, not final RTT analysis.
...
- Sources:
  - [Audioscope repo](https://github.com/guglielmocamporese/audioscope), checked 2026-06-15
...
- Relevant idea: Collect post-block SA3 DiT residual activations, build
  ...
  residual steering.
- Source-confirmed detail: Audioscope reports a mood direction from
  mean-difference residual activations, layer-wise logistic probing, and
  multi-layer steering. Its README also notes that `register_forward_hook`
  collection is not enough for compiled-block intervention; steering is done by
  patching each block's `forward` method during generation.
...
- Pitch-relation note: this is useful background for how SA3 residuals can be
  captured and patched, but the tuning project should start from audio-measured
  `TuningMap` targets rather than prompt-pair contrast labels.
