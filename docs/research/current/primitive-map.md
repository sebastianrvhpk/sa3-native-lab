...
| Control-lane mechanistic probes | `control_lane_layer_probe_rows(...)`, `control_lane_window_probe_rows(...)`, `control_lane_timestep_probe_rows(...)`, `control_lane_null_layer_probe_rows(...)`, `control_lane_probe_prediction_table(...)`, `control_lane_active_direction_table(...)`, `SA3ControlLaneProbeExtractor(...).probe_audio_path(...)` | control lanes + captured SA3 residual activations -> lane/layer, lane/window, lane/timestep, null-control, prediction, and active-direction rows | microscope/selector; not causal until residual patches move decoded audio |
...
| `control_lane_probes.py` | confirmed | Continuous control-lane-to-residual linear probes, blocked-CV rows, observed-call window rows, sampler-timestep rows, null controls, held-out prediction rows, and active/quiet direction previews. |
...
| `procedures/control_lane_mechanistic_probe.py` | confirmed | Audio-conditioned SA3 residual capture plus lane/layer and lane/window probe packaging. |
