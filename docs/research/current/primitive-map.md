...
| Control-lane mechanistic probes | `control_lane_layer_probe_rows(...)`, `control_lane_window_probe_rows(...)`, `control_lane_timestep_probe_rows(...)`, `control_lane_null_layer_probe_rows(...)`, `control_lane_null_margin_table(...)`, `control_lane_probe_prediction_table(...)`, `control_lane_active_direction_table(...)`, `SA3ControlLaneProbeExtractor(...).probe_audio_path(...)` | control lanes + captured SA3 residual activations -> lane/layer, lane/window, token-preserving lane/timestep, null-control, true-vs-null margin, prediction, and active-direction rows | microscope/selector; not causal until residual patches move decoded audio |
