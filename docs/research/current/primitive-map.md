...
| `control_lane_probes.py` | confirmed | Continuous control-lane-to-residual linear probes, token-blocked and call-held-out CV rows, observed-call window rows, sampler-timestep rows, null controls, null-margin rows, held-out prediction rows, and active/quiet direction previews. |
