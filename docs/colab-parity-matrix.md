...
| 0c | Latent-selective renoise | Morph gesture, `latent.renoise` | native | `source latent`, `mode`, `fraction`, `sigma`, channel mask fields, `seed`, `backend` | Latent artifact, recipe, selected-mask metadata | True bounded time-region masks are not backend-supported | Latent-region model and RecipeFields tests exist | Add donor/listening comparison after branch use |
| 0c-search | Annotation retrieval | Artifact annotation, Memory roles/reuse intent, Memory browser, archive filters | native | label, notes, tags, decision metadata, memory role, reuse intent, text/tag/model/gesture/branch filters | Updated artifact metadata and searchable archive/memory shelf | Rating-specific UI is not first-class | Filter tests, memory browser/model tests, SessionTray reuse tests, first-use smoke exist; rating/search tests missing | Keep rating-specific UI deferred until listening use requires it |
| 0e | Cross-audio latent channel graft | Borrow Texture gesture, `latent.graft`, donor latent picker, remembered latent donor reuse, channel-mask summary | native | `source latent`, `donor latent`, `mode`, `fraction`, `amount`, channel mask fields, `seed`, `backend` | Latent artifact, source/donor lineage, recipe | Donor preview and true time-region selection | Memory donor availability and latent-region tests exist | Add donor audition comparison after branch/sweep comparison |
...
| 2 | Audio to babble / hard prompt | `experiment.prompt_search` covers hard-token search | partial | target audio, vocabulary, separator, search mode, prompt probe, tokens, beam, samples, seed, t/logSNR | Prompt-search bundle, prompt candidates, generated takes, run-comparison rows | Hard-token preset, probe-cost guidance, vocabulary sets, and prompt history exist; vocabulary editing is still preset-based. Selectable probes are `lexical_probe` and `sa3_flow_probe`. | Prompt-search helper, run-comparison, preset, probe-note, vocabulary, history, and backend spec tests exist | Add custom vocabulary save/share later |
| 3 | Audio to readable prompt | `experiment.prompt_search` with modifier axes | partial | target audio, modifier axes, separator, search rounds, prompt probe, prefix/suffix | Prompt JSON bundle, generated takes, run-comparison rows | Readable-prompt preset, probe-cost guidance, axis sets, and prompt history exist; axes editing is still preset-based. Selectable probes are `lexical_probe` and `sa3_flow_probe`. | Preset/run-comparison/probe-note/axis/backend-spec tests exist | Add custom axis save/share later |
...
| 13 | Fine-tuning scaffold | External: [dada-bots/underfit](https://github.com/dada-bots/underfit) on Colab A100 | delegated | dataset/style/domain adaptation | external checkpoints/logs | not owned by SA3 Native Lab | not applicable | Keep out of the local product loop |
| 14 | Latent memory instrument | Memory shelf, Memory browser, active reuse actions, latent import/encode, dataset pre-encode, `memory.query` | partial | source latent, top K, metric, exclude self, encoded datasets, memory role/reuse intent | Memory-result bundle, ranked hits, remembered artifacts, reuse actions, recovery | Dataset browser, preview audio, style/reference promotion | Memory model/browser tests, SessionTray reuse tests, first-use smoke exist | Add encoded-dataset/source preview before vector search |
...
4. Use the Memory browser/filter pass as the baseline for remembered material:
   role, reuse intent, tags, notes, kind, source, decision, and branch are now
   product-level metadata, not a future vector-search prerequisite.
...
- What is the minimum useful Postgres schema once JSON manifests become too
  ...
  presets first?
- Which Medium/MPS prompt-probe settings are cheap enough to recommend as defaults
