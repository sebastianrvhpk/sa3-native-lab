...
| Custom Colab audio player | Listening bench, waveform peaks, region loop, A/B, decisions | partial | Player is still more utilitarian than creative; session playlist/take stack/markers are missing. | Playback interaction tests are thin. | P1 playback-composer pass after parameter parity. |
...
| 2 | Audio to babble / hard prompt | `experiment.prompt_search` covers hard-token search | partial | target audio, vocabulary, search mode, scorer, tokens, beam, samples, seed, t/logSNR | Prompt-search bundle, prompt candidates, generated takes | Clear Mode 2 preset and hard-token vocabulary inspector | Prompt-search helper tests exist; Mode 2 preset tests missing | Add mode preset and comparison rows |
| 3 | Audio to readable prompt | `experiment.prompt_search` with modifier axes | partial | target audio, modifier axes, search rounds, scorer, prefix/suffix | Prompt JSON bundle and generated takes | Readable-prompt preset, axes editor, score explanation | Modifier-axis tests missing | Add readable prompt preset after Mode 2 comparison |
...
| 8 | SA3 residual steering from prompts | prompt residual vectors, alpha sweep | recipe | axis, prompt pairs, layers, alphas, prompt, generation fields | Vector bundle and sweep family | Layer/alpha comparison and prompt-pair editor | Sweep tests exist; layer comparison missing | P1 prompt/residual comparison pass |
...
| 11 | Inpainting / continuation composition | `generate.inpaint`, `generate.audio_to_audio` | native | source audio, prompt, duration, seed, range, init noise, model/decoder | Audio artifact, recipe, source lineage | Composition timeline and continuation chain view | Inpaint contract tests exist; timeline tests missing | Add after playback/timeline upgrade |
...
3. Add prompt-search layer/scorer comparison rows next, because Modes 2, 3, 8,
   and 9 all benefit from the same comparison surface.
...
5. Add session/workspace/archive modeling before Postgres so the persistence
   schema stores the right product objects.
