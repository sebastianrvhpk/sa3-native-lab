...
from latent_audio_primitives.style import apply_style_direction, load_style_direction
from _runtime import add_torch_runtime_args, model_half_from_args


def main() -> None:
    parser = argparse.ArgumentParser(description="Generate SA3 latents, apply a target-reference SAME style direction, decode audio.")
    ...
    parser.add_argument("--std-alpha", type=float, default=0.0)
    ...
    styled = _apply_direction_to_batch(latents, direction, alpha=args.alpha, std_alpha=args.std_alpha)
...
def _apply_direction_to_batch(latents, direction, *, alpha: float, std_alpha: float):
    import torch
    ...
    for latent in arr:
        time_major = latent.T
        styled_time_major = apply_style_direction(time_major, direction, alpha=alpha, std_alpha=std_alpha)
