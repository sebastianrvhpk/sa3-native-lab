...
LoRA is not an active local notebook mode in SA3 Native Lab. Use
...
This notebook can still compare audio exported from an Underfit run through the
ordinary descriptor, memory, player, and annotation cells. It should not grow
local LoRA training cells, checkpoint dashboards, or adapter-control plumbing.

Recommended handoff boundary:

```text
local dataset / selected clips
  -> optional prompt and metadata notes outside this notebook
  -> Underfit LoRA run
  -> exported audio examples and run notes
  -> SA3 Native Lab descriptor/listening comparison
```

Keep this mode as a boundary marker only.
"""
    ),
    md(
        r"""
...
manifest = {
    ...
    "modes": {
        ...
        "13_external_lora_handoff": "underfit",
